...
import { useMySalesTargetStore } from '@/stores/mySalesTargetStore'
import { useToast } from 'vue-toastification'

const store = useMySalesTargetStore()
const toast = useToast()

const showYearMenu = ref(false)

// nama sales diambil dari baris target itu sendiri (semua row pasti
// sales_name-nya sama, karena endpoint /sales-targets sudah di-scope ke
// user yang login lewat canManageTargets() di controller) -- biar nggak
// perlu gantung ke authStore/profile yang bentuknya belum tentu sama.
...
onMounted(async () => {
  try {
    ...
    toast.error('Gagal memuat target penjualan.')
  }
})

function changeYear(year) {
  showYearMenu.value = false
  store.changeYear(year)
}
...
function isDone(row) {
  return (row.achievement_percent ?? 0) >= 100
}
</script>
    ...
    <div class="toolbar-top">
      <div class="drop-wrap">
        <button class="btn-select" @click="showYearMenu = !showYearMenu">
          <font-awesome-icon icon="calendar" /> Tahun {{ store.periodYear }}
        ...
        <div class="drop-menu" :class="{ show: showYearMenu }">
          ...
          <button v-for="y in store.yearOptions" :key="y" class="drop-item"
            :class="{ active: store.periodYear === y }"
        ...
        <div v-else v-for="row in store.sortedTargets" :key="row.id" class="target-card" :class="{ done: isDone(row) }">
          <div class="target-card-top">
            ...
            <span class="result-chip" :class="isDone(row) ? 'status-done' : 'status-pending'">
              <font-awesome-icon :icon="isDone(row) ? 'circle-check' : 'hourglass-half'" />
              {{ isDone(row) ? 'Tercapai' : 'Berjalan' }}
            </span>
          </div>
          ...
          <div class="target-progress-row">
            <div class="progress-bar-track">
              <div class="progress-bar-fill" :class="{ done: isDone(row) }" :style="{ width: Math.min(row.achievement_percent, 100) + '%' }"></div>
            </div>
            <span class="progress-cell-label">
              {{ store.formatCurrency(row.achieved_amount) }} / {{ store.formatCurrency(row.target_amount) }} &middot; {{ row.achievement_percent }}%
            </span>
          </div>
...
/* ── dropdown tahun (pola sama kayak SalesTargetManagement.vue) ── */
.drop-wrap { position: relative; }
.btn-select {
  display: flex; align-items: center; gap: 8px; padding: 8px 14px; border-radius: 8px; border: 1px solid var(--border-main);
  background: var(--bg-input); color: var(--text-primary); font-size: 0.85rem; font-weight: 600; cursor: pointer;
}
.btn-arrow { font-size: 0.7rem; margin-left: 2px; color: var(--text-muted); }
.drop-menu {
  position: absolute; top: calc(100% + 6px); left: 0; min-width: 160px; background: var(--bg-card); border: 1px solid var(--border-main);
  border-radius: 10px; box-shadow: 0 6px 18px rgba(0,0,0,0.12); padding: 6px; z-index: 20; display: none; flex-direction: column; gap: 2px;
}
.drop-menu.show { display: flex; }
.drop-label { font-size: 0.7rem; color: var(--text-muted); font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; padding: 4px 8px; }
.drop-item { text-align: left; background: transparent; border: none; padding: 8px 10px; border-radius: 6px; font-size: 0.85rem; color: var(--text-primary); cursor: pointer; }
.drop-item:hover { background: var(--bg-input); }
.drop-item.active { background: rgba(99,102,241,0.12); color: #6366f1; font-weight: 700; }
...
.hero-stat-label { font-size: 0.76rem; color: var(--text-muted); font-weight: 500; max-width: 220px; }
...
.target-card-name { font-size: 0.95rem; font-weight: 700; color: var(--text-primary); overflow-wrap: break-word; }

.activity-badge {
  display: inline-flex; align-items: center; gap: 5px; font-size: 0.7rem; font-weight: 700; padding: 3px 9px; border-radius: 20px;
  min-width: 0; flex: 1 1 auto; overflow: hidden;
}
.activity-badge-text { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

.result-chip { display: inline-flex; align-items: center; gap: 5px; font-size: 0.7rem; font-weight: 700; padding: 3px 9px; border-radius: 20px; white-space: nowrap; flex-shrink: 0; }
