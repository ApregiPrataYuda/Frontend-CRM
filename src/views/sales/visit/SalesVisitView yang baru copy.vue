...
const customerCheckOutForm = ref(emptyCustomerCheckOutForm())

// ── Follow Up: sekarang pakai datetime-local, minimal besok jam 00:00 ──
...
// ── Helper: cek apakah isi rich text (HTML dari Tiptap) kosong ──
...
const isCustomerCheckOutValid           = computed(() => {
  ...
  if (!f.no_reference.trim() || isRichTextEmpty(f.notes) || !f.customer_response || !f.follow_up_at || !f.follow_up_type) return false
  if (f.has_complaint       && isRichTextEmpty(f.complaint_detail))       return false
  if (f.has_potential_order && isRichTextEmpty(f.potential_order_detail)) return false
  return true
})
...
// ── DRAFT PERSISTENCE ──────────────────────────────────
// Menyimpan isi form check out customer yang BELUM di-submit, per customer/branch,
// supaya kalau modal ditutup (X / backdrop / Cancel) lalu dibuka lagi, datanya masih ada.
const checkOutDrafts = ref({}) // key → { form, file }
...
function openCustomerCheckOut(item) {
  ...
  customerCheckOutForm.value = draft ? { ...draft.form } : emptyCustomerCheckOutForm()
  selectedCheckOutFile.value = draft?.file ?? null

  showCheckOutCustomerModal.value = true
}

// Ditutup lewat X / backdrop / Cancel → SIMPAN draft, jangan hilang
function closeCustomerCheckOutModal() {
  ...
  if (selectedCustomerCheckOut.value) {
    checkOutDrafts.value[draftKey(selectedCustomerCheckOut.value)] = {
      form: { ...customerCheckOutForm.value },
      file: selectedCheckOutFile.value,
    }
  }
...
// Dipakai setelah submit BERHASIL → form benar-benar dikosongkan, draft dihapus
function hardCloseCustomerCheckOutModal() {
  showCheckOutCustomerModal.value = false
  selectedCustomerCheckOut.value  = null
  selectedCheckOutFile.value      = null
  customerCheckOutForm.value      = emptyCustomerCheckOutForm()
}
    ...
    >
      ...
      <template v-else-if="visitsDetail">
        <div class="detail-list">
          ...
          <div class="detail-row">
            <span class="detail-label">No Reference</span>
            <span class="detail-value mono">{{ visitsDetail.no_reference }}</span>
          </div>
          <div class="detail-row">
          ...
          <div v-if="visitsDetail.check_out_file" style="margin-top:8px">
            ...
            <img
              ...
              style="width:100%; border-radius:10px; object-fit:contain; max-height:280px; cursor: zoom-in;"
              @click="openImageZoom(photoBase + visitsDetail.check_out_file)"
            />
          ...
          <div v-if="visitsDetail.photo" style="margin-top:8px">
            ...
            <img
              :src="photoBase + visitsDetail.photo"
              style="width:100%; border-radius:10px; object-fit:contain; max-height:280px; cursor: zoom-in;"
              @click="openImageZoom(photoBase + visitsDetail.photo)"
            />
          </div>
    ...
    >
      <div class="form-container-gap">
        ...
        <div class="form-group">
          ...
          <RichTextEditor v-model="customerCheckOutForm.notes" placeholder="Write visit result here..." />
        </div>
        ...
        <div v-if="showCustomerComplaintSection" class="form-group">
          ...
          <RichTextEditor v-model="customerCheckOutForm.complaint_detail" placeholder="Describe customer complaint or detail Issue requires further attention and etc..." />
        </div>
        ...
        <div v-if="showCustomerPotentialOrderSection" class="form-group">
          ...
          <RichTextEditor v-model="customerCheckOutForm.potential_order_detail" placeholder="Explain. Write additional notes. Detail examples (potential orders or future plans)" />
        </div>

        <!-- Follow Up -->
        <div class="form-group">
          <label>Follow Up Date / Next Action Plan Date <span style="color:#ef4444">*</span></label>
          <input v-model="customerCheckOutForm.follow_up_at" type="datetime-local" :min="tomorrowDateTime" class="form-input" />
        </div>
        ...
        <div class="form-group">
          ...
          <RichTextEditor v-model="customerCheckOutForm.follow_up_notes" placeholder="Write next follow up notes..." />
        </div>

        <!-- Progress indicator -->
        <div style="display:flex; align-items:center; gap:8px; font-size:0.78rem; color:var(--text-muted)">
          <span style="width:8px;height:8px;border-radius:50%;display:inline-block" :style="!isRichTextEmpty(customerCheckOutForm.notes) ? 'background:#10b981' : 'background:#e2e8f0'"></span>
    ...
    <Teleport to="body">
      <div v-if="showImageZoom" class="lightbox-overlay" @click.self="closeImageZoom">
        <div class="lightbox-toolbar">
          <button class="lightbox-btn" @click="zoomOut" title="Zoom Out">
            <font-awesome-icon icon="magnifying-glass-minus" />
          </button>
          <span class="lightbox-zoom-label">{{ Math.round(zoomLevel * 100) }}%</span>
          <button class="lightbox-btn" @click="zoomIn" title="Zoom In">
            <font-awesome-icon icon="magnifying-glass-plus" />
          </button>
          <button class="lightbox-btn" @click="resetZoom" title="Reset">
            <font-awesome-icon icon="arrows-rotate" />
          </button>
          <button class="lightbox-btn lightbox-close" @click="closeImageZoom" title="Close">
        ...
        >
          <img
            ...
            class="lightbox-img"
            :style="{
              transform: `translate(${zoomPos.x}px, ${zoomPos.y}px) scale(${zoomLevel})`,
