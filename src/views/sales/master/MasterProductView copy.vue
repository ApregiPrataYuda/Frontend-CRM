...
async function handleSyncCustomers() {
  ...
  try {
    // TODO: ganti dengan panggilan API sync yang sebenarnya
...
const menus = ref([
  { id: 1, sku: '992810092', name: 'admin', price: '13.000', cost: '12.000', category: 'pump', uom: 'test', barcode: 'q', stock: '12', active: 'active' },
])
...
function submitAddData() {
  ...
  if (isEdit.value && selectedEditMenu.value) {
    ...
    menus.value.push({
      ...
      created: new Date().toLocaleDateString('en-EN', { day: 'numeric', month: 'long', year: 'numeric' }),
    })
  }
  closeAddModal()
}
      ...
      <div class="controls-row">
        <div class="controls-left">
          ...
          <button class="btn-toolbar btn-purple" :disabled="syncingCustomers" @click="handleSyncCustomers">
            ...
            {{ syncingCustomers ? 'Syncing...' : 'Sync Product' }}
          </button>
        </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else v-for="(menu, index) in paginatedRows" :key="menu.id" class="data-row">
            ...
            <td class="td-name">{{ menu.sku }}</td>
            <td class="td-name">{{ menu.name }}</td>
            <td class="td-name">{{ menu.price }}</td>
            <td class="td-name">{{ menu.cost }}</td>
            <td class="td-name">{{ menu.category }}</td>
            <td class="td-name">{{ menu.uom }}</td>
            <td class="td-name">{{ menu.barcode }}</td>
            <td class="td-name">{{ menu.stock }}</td>
            <td class="td-name">{{ menu.active }}</td>
            <td class="td-actions">
