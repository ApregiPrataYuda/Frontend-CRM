...
export const masterLeadsStore = defineStore('leads', () => {
  ...
  // ── SELECT: COMPANY NAME (autocomplete cek duplikat) ──
  const companyNameSelectData = ref([]);
  let companySearchTimeout = null;

  const fetchCompanyNameSelect = (keyword) => {
    clearTimeout(companySearchTimeout);
    if (!keyword) {
      companyNameSelectData.value = [];
      return;
    }
    companySearchTimeout = setTimeout(async () => {
      try {
        const response = await masterLeadsServices.searchCompanyName(keyword);
        companyNameSelectData.value = response.data.data ?? [];
      } catch (error) {
        console.error("Gagal fetch company name:", error);
      }
    }, 400);
  };
